...
if login()==False:
    st.stop() 
    # Place the rest of your app logic here
...
# Logo image path fix
st.image(r"assets/logo.png", width=250)  # Ensure correct path
...
# Rest of your functions (same as in your original code)...
...
def enhanced_vegetation_analysis_tab():
    ...
    with st.sidebar:
        # st.image(r"assets\logo.png", width=400,clamp=True) 
                ...
                # Add temperature data if requested
                    if show_temperature:
                        with st.spinner("Processing temperature data..."):
                            ...
                            # Calculate min and max temperature values for visualization (like GEE implementation)
                            stats = temp_data.reduceRegion(
                            ...
                            # Evaluate the stats dictionary to get min and max values
                            min_temp = stats.get('temp_C_min').getInfo()
                            max_temp = stats.get('temp_C_max').getInfo()
                            
                            # Use dynamic min_temp and max_temp for visualization parameters
                            temp_vis = {
                                'min': min_temp,
                                'max': max_temp,
